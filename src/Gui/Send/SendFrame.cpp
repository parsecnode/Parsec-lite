...
void SendFrame::sendClicked() {
  ...
  QuestionDialog confirmDlg(QString(tr("Confirm sending %1 PARS")).arg(m_cryptoNoteAdapter->formatUnsignedAmount(transferSum)),
    m_ui->m_paymentIdEdit->text().isEmpty() ? QString(tr("<html><head/><body><p>Are you sure you want to send <strong>%1 PARS</strong> "
    "<strong>without Payment ID?</strong></p></body></html>")).arg(m_cryptoNoteAdapter->formatUnsignedAmount(transferSum)) :
    QString(tr("<html><head/><body><p>Are you sure you want to send <strong>%1 PARS</strong> with Payment ID:</p>"
