...
const char DEFAULT_WALLET_FILE_NAME[] = "parsec.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultNodeList << "45.76.124.54:32616" << "207.246.127.59:32616";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("parsecwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("parsecwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("parsecwallet.desktop");
  ...
  res = autorunSettings.contains("parsecwallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("parsecwallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "com.parsecnode.parsecwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Parsec Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("parsecwallet", appPath);
    } else {
      autorunSettings.remove("parsecwallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\parsec", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:parsecnode");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\parsec\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\parsec\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("parsecwallet.cfg"));
